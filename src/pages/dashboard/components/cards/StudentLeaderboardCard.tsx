  ...
  return (
    ...
    >
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div className="inline-flex w-full sm:w-auto rounded-full border border-[rgb(var(--border))] bg-[rgb(var(--surface))] p-1">
          <button
            type="button"
            onClick={() => setMode("rank")}
            className={cn(
              "flex-1 sm:flex-none rounded-full px-3 py-1.5 text-xs font-semibold",
              mode === "rank" ? activeBtn : idleBtn
            )}
          >
            Rank
          </button>
      ...
      <div className="mt-3 overflow-x-auto rounded-2xl border border-[rgb(var(--border))]">
        <table className="w-full min-w-[360px] sm:min-w-[420px] text-left text-xs">
          <thead className="bg-[rgb(var(--surface-2))]">
            <tr>
          ...
          <tbody>
            {rows.map((r) => (
              <tr
                ...
                className="border-t border-[rgb(var(--border))] hover:bg-[rgb(var(--surface-2))]/60"
              >
                <td className="px-3 py-2 text-[rgb(var(--text))] whitespace-nowrap">{r.rank}</td>
                <td className="px-3 py-2 text-[rgb(var(--text))]/85">
                  <span className="line-clamp-1">{r.student}</span>
                </td>
                <td className="px-3 py-2 text-[rgb(var(--text))] whitespace-nowrap">{r.score}</td>
              </tr>
            ))}
          </tbody>
        </table>
